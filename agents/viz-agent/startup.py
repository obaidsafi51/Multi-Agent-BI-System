#!/usr/bin/env python3
"""
Viz Agent Startup Script

Starts both HTTP API server and WebSocket server concurrently.
This ensures the Viz agent can handle both traditional HTTP requests 
...
class VizAgentLauncher:
    ...
    def __init__(self):
        ...
        self.http_port = int(os.getenv("PORT", "8003"))
        self.websocket_enabled = os.getenv("ENABLE_WEBSOCKETS", "true").lower() == "true"
        self.websocket_host = os.getenv("WEBSOCKET_HOST", "0.0.0.0")
        self.websocket_port = int(os.getenv("WEBSOCKET_PORT", "8013"))
        
        logger.info(f"Viz Agent Configuration:")
    ...
    async def start_websocket_server(self):
        ...
        try:
            ...
            # Import here to avoid circular imports
            from websocket_server import WebSocketVizServer
            
            # Create and start WebSocket server
            ws_server = WebSocketVizServer(
    ...
    async def run(self):
        ...
        try:
            logger.info("Starting Viz Agent with HTTP and WebSocket servers...")
            ...
            logger.error(f"Viz Agent startup failed: {e}")
            raise
        finally:
