...
# Import MCP schema management components (optional dependency)
try:
    from ..schema_management.manager import MCPSchemaManager
    from ..schema_management.enhanced_data_validator import EnhancedDataValidator, EnhancedFinancialDataValidator
    from ..schema_management.dynamic_validator import DynamicValidationConfig
    MCP_AVAILABLE = True
except ImportError:
    logger.warning("MCP schema management not available - using static validation only")
...
async def validate_data_quality_with_mcp(
    ...
    table_name: str,
    database_name: str = "financial_db",
    schema_manager: Optional[MCPSchemaManager] = None
) -> Tuple[List[Dict[str, Any]], List[str]]:
    """
    Validate data quality using MCP schema management with fallback to static validation.
    
    Args:
        data: List of data records
        table_name: Name of the target table
        database_name: Name of the database
        schema_manager: MCP schema manager instance (optional)
        
    Returns:
        Tuple of (validated_data, warnings)
    """
    if not MCP_AVAILABLE or not schema_manager:
        logger.info("MCP not available, falling back to static validation")
        return validate_data_quality(data, table_name)
    ...
    try:
        ...
        for i, record in enumerate(data):
            try:
                ...
                # Fallback to static validation for this record
                try:
                    if table_name in ['financial_overview', 'cash_flow', 'budget_tracking', 'investments']:
                        validator_map = {
                            'financial_overview': FinancialDataValidator.validate_financial_overview,
                            'cash_flow': FinancialDataValidator.validate_cash_flow,
                            'budget_tracking': FinancialDataValidator.validate_budget_tracking,
                        ...
                        validator = validator_map[table_name]
                        validated_record = validator(record)
                        validated_data.append(validated_record)
                    else:
                        validated_data.append(record)
                    ...
                    warnings.append(f"Record {i+1}: Static validation also failed - {static_error}")
        ...
        # Fallback to static validation for all data
        return validate_data_quality(data, table_name)
...
class MCPIntegratedDataValidator:
    """
    Data validator that integrates MCP schema management with existing validation logic.
    
    This class provides a bridge between the existing validation system and the new
    MCP-based dynamic validation, maintaining backward compatibility while adding
    enhanced capabilities when MCP is available.
    """
    ...
    def __init__(self, schema_manager: Optional[MCPSchemaManager] = None):
        ...
        if MCP_AVAILABLE and schema_manager:
            self.enhanced_validator = EnhancedDataValidator(schema_manager)
            logger.info("MCP integrated data validator initialized with schema manager")
        else:
            logger.info("MCP integrated data validator initialized in static mode")
    ...
    async def validate_with_schema(
        ...
        """
        Validate data using MCP schema information with fallback to static validation.
        ...
        if self.enhanced_validator:
            try:
                ...
                logger.error(f"MCP validation failed, falling back to static: {e}")
                # Fall through to static validation
        
        # Static validation fallback
        return self._static_validation_fallback(data, table)
    
    def _static_validation_fallback(self, data: Dict[str, Any], table: str) -> Dict[str, Any]:
        """Fallback to static validation."""
        if table in ['financial_overview', 'cash_flow', 'budget_tracking', 'investments']:
            validator_map = {
                'financial_overview': FinancialDataValidator.validate_financial_overview,
                'cash_flow': FinancialDataValidator.validate_cash_flow,
                'budget_tracking': FinancialDataValidator.validate_budget_tracking,
            ...
            validator = validator_map.get(table)
            if validator:
                return validator(data)
        
        # For unknown tables, just return the data as-is
        logger.warning(f"No validator available for table '{table}', returning data as-is")
        return data
    
    # Backward compatibility methods that delegate to static validators
    
    def validate_decimal(self, value: Any, field_name: str, max_digits: int = 15, decimal_places: int = 2):
        """Backward compatibility wrapper for decimal validation."""
        return DataValidator.validate_decimal(value, field_name, max_digits, decimal_places)
    
    def validate_date(self, value: Any, field_name: str):
        """Backward compatibility wrapper for date validation."""
    ...
    def validate_string(self, value: Any, field_name: str, max_length: int = None, min_length: int = 0):
        """Backward compatibility wrapper for string validation."""
        return DataValidator.validate_string(value, field_name, max_length, min_length)
    
    def validate_percentage_typed(self, value: Any, field_name: str, percentage_type: PercentageType = PercentageType.STANDARD):
        """Backward compatibility wrapper for percentage validation."""
        return DataValidator.validate_percentage_typed(value, field_name, percentage_type)
    
    def validate_percentage(self, value: Any, field_name: str):
        """Backward compatibility wrapper for percentage validation."""
        return DataValidator.validate_percentage(value, field_name)
    
    def validate_positive_decimal(self, value: Any, field_name: str):
        """Backward compatibility wrapper for positive decimal validation."""
        return DataValidator.validate_positive_decimal(value, field_name)
    
    def validate_user_id(self, value: Any):
        """Backward compatibility wrapper for user ID validation."""
        return DataValidator.validate_user_id(value)
    
    def validate_json_data(self, value: Any, field_name: str):
        """Backward compatibility wrapper for JSON validation."""
        return DataValidator.validate_json_data(value, field_name)
...
class MCPIntegratedFinancialDataValidator:
    ...
    Provides enhanced financial validation with real-time schema information
    ...
    def __init__(self, schema_manager: Optional[MCPSchemaManager] = None):
        ...
        if MCP_AVAILABLE and schema_manager:
            ...
            logger.info("MCP integrated financial validator initialized with schema manager")
        else:
            logger.info("MCP integrated financial validator initialized in static mode")
    
    async def validate_financial_overview(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate financial overview data with MCP integration."""
        if self.enhanced_validator:
            try:
                return await self.enhanced_validator.validate_financial_overview(data)
            except Exception as e:
                logger.error(f"MCP financial validation failed, falling back to static: {e}")
        
        # Fallback to static validation
        return FinancialDataValidator.validate_financial_overview(data)
    
    async def validate_cash_flow(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate cash flow data with MCP integration."""
        if self.enhanced_validator:
            try:
                return await self.enhanced_validator.validate_cash_flow(data)
            except Exception as e:
                logger.error(f"MCP cash flow validation failed, falling back to static: {e}")
        
        return FinancialDataValidator.validate_cash_flow(data)
    
    async def validate_budget_tracking(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate budget tracking data with MCP integration."""
        if self.enhanced_validator:
            try:
                return await self.enhanced_validator.validate_budget_tracking(data)
            except Exception as e:
                logger.error(f"MCP budget validation failed, falling back to static: {e}")
        
        return FinancialDataValidator.validate_budget_tracking(data)
    
    async def validate_investment(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Validate investment data with MCP integration."""
        if self.enhanced_validator:
            try:
                return await self.enhanced_validator.validate_investment(data)
            except Exception as e:
                logger.error(f"MCP investment validation failed, falling back to static: {e}")
        
        return FinancialDataValidator.validate_investment(data)
    
    # Synchronous methods for backward compatibility
    
    def validate_financial_overview_sync(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Synchronous financial overview validation (static only)."""
        return FinancialDataValidator.validate_financial_overview(data)
    
    def validate_cash_flow_sync(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Synchronous cash flow validation (static only)."""
        return FinancialDataValidator.validate_cash_flow(data)
    
    def validate_budget_tracking_sync(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Synchronous budget tracking validation (static only)."""
        return FinancialDataValidator.validate_budget_tracking(data)
    
    def validate_investment_sync(self, data: Dict[str, Any]) -> Dict[str, Any]:
        """Synchronous investment validation (static only)."""
        return FinancialDataValidator.validate_investment(data)


# Factory functions for creating MCP-integrated validators

def create_mcp_data_validator(schema_manager: Optional[MCPSchemaManager] = None) -> MCPIntegratedDataValidator:
    """
...
def create_mcp_financial_validator(schema_manager: Optional[MCPSchemaManager] = None) -> MCPIntegratedFinancialDataValidator:
    """
