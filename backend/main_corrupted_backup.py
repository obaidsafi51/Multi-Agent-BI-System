...
async def get_database_list(request: Request):
            # Use WebSocket for standard NLP processing
            try:
                nlp_result = await send_to_agent_enhanced(
                    AgentType.NLP,
                    {
                ...
                logger.warning(f"WebSocket communication failed, falling back to HTTP: {ws_error}")
        ...
        try:
            # Use WebSocket for data agent communication
            data_result = await send_to_agent_enhanced(
                AgentType.DATA,
                {
                    "type": "data_query",
            ...
            logger.info(f"📊 Data Agent WebSocket response received: {data_result.get('success', False)}")
        except Exception as e:
            logger.warning(f"❌ Data Agent WebSocket communication error, trying HTTP fallback: {e}")
            try:
                data_result = await send_to_data_agent(sql_query, query_intent, query_id, session_id)
                logger.info(f"📊 Data Agent HTTP fallback response received: {data_result.get('success', False)}")
            except Exception as http_error:
                logger.error(f"❌ Data Agent HTTP fallback also failed: {http_error}")
        ...
        try:
            # Use WebSocket for viz agent communication
            viz_result = await send_to_agent_enhanced(
                AgentType.VIZ,
                {
                    "type": "viz_query",
            ...
            logger.info(f"📈 Viz Agent WebSocket response received: {viz_result.get('success', False)}")
        except Exception as e:
            logger.warning(f"⚠️ Viz Agent WebSocket communication error, trying HTTP fallback: {e}")
            try:
                viz_result = await send_to_viz_agent(query_data, query_intent, query_id, session_id)
                logger.info(f"📈 Viz Agent HTTP fallback response received: {viz_result.get('success', False)}")
            except Exception as http_error:
                logger.warning(f"⚠️ Viz Agent HTTP fallback also failed: {http_error}")
...
# WebSocket endpoint for real-time BI queries and communication
...
async def handle_websocket_query(websocket: WebSocket, user_id: str, data: Dict[str, Any]):
    ...
    try:
        ...
        database_context = data.get("database_context", {})
